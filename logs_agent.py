...
class KQLAgent:
    ...
    async def call_mcp_tool(
        ...
        try:
            if tool_name == "execute_kql_query":
                ...
                # Execute via canonical helper
                exec_result = execute_kql_query(kql=query, workspace_id=workspace_id, client=client, timespan=timespan)
                # Compatibility: `exec_result['exec_stats']['status']` is a
                # normalized upper-case string (e.g. "SUCCESS"). The SDK enum
                # may be provided as `raw_status` for advanced callers.
                # Prefer `utils.kql_exec.is_success(...)` for robust checks.
                tables = exec_result.get("tables", [])
                if tables:
                    return {"success": True, "tables": tables}
                else:
                    err = exec_result.get("exec_stats", {}).get("error", "Query failed")
                ...
                if filename and os.path.exists(filename):
                    ...
                    return {
                        "success": False,
                        "error": f"No examples found for scenario: {scenario}",
                    }
                ...
                try:
                    exec_result = execute_kql_query(kql=test_query, workspace_id=workspace_id, client=client, timespan=None)
                    tables = exec_result.get("tables", [])
                    if tables:
                        return {"success": True, "message": f"✅ Successfully connected to workspace: {workspace_id}"}
                    else:
                        err = exec_result.get("exec_stats", {}).get("error", "Unknown error")
                        return {"success": False, "error": f"❌ Failed to connect: {err}"}
                except Exception as e:
    ...
    def format_table_results(self, tables: List[Dict]) -> Dict:
        ...
        if not tables:
            return {"type": "no_data", "message": "No data returned"}
    ...
    def detect_query_timespan(self, kql_query):
        ...
        has_time_filter = any(
            pattern in query_lower for pattern in time_filter_patterns
        )
    ...
    async def process_natural_language(self, question: str) -> str:
            ...
            for scenario in scenarios:
                if scenario in question_lower:
                    ...
                    result = await self.call_mcp_tool(
                        "get_kql_examples", {"scenario": scenario}
                    )
        ...
        if "test" in question_lower and (
            "connection" in question_lower or "workspace" in question_lower
        ):
            print("🔗 Testing workspace connection...")
            result = await self.call_mcp_tool(
                "validate_workspace_connection", {"workspace_id": self.workspace_id}
            )
    ...
    async def explain_results(
        self, query_result: Dict, original_question: str = ""
    ) -> str:
        """
        ...
        try:
            ...
            data_summary = self._format_data_for_explanation(
                truncated_tables, query_result.get("kql_query", "")
            )

            # Call OpenAI to explain the results
            explanation = await self._call_openai_for_explanation(
                data_summary, original_question
            )
    ...
    async def _call_openai_for_explanation(
        self, data_summary: str, original_question: str
    ) -> str:
        ...
        try:
            ...
            from azure_openai_utils import (emit_chat_event, get_env_int,
                                            run_chat, truncate_text)
            ...
            # Limits
            max_data_chars = get_env_int(
                "AZURE_OPENAI_EXPLAIN_MAX_DATA_CHARS",
                8000,
                min_value=1000,
                max_value=20000,
            )
            if len(data_summary) > max_data_chars:
                print(
                    f"[Explain Debug] Truncating data_summary from {len(data_summary)} to {max_data_chars} chars"
                )

...
class KqlStage:
    ...
    def _parse_join(self, content: str) -> Dict[str, Any]:
        ...
        if on_match:
            ...
            on_conditions = [normalize_whitespace(c.strip()) for c in on_clause.split(',')]
    ...
    for stage_type in all_types:
        ...
        if not exp_list and gen_list:
            ...
            if stage_type == 'sort' and _is_default_time_sort(gen_list):
                ...
                result['details']['differences'].append(f'Extra {stage_type} stage(s) in generated')
                scores.append((weights.get(stage_type, 0.05), 0.0))
        elif exp_list and not gen_list:
            # Missing stages in generated
            result['details']['differences'].append(f'Missing {stage_type} stage(s) in generated')
            scores.append((weights.get(stage_type, 0.05), 0.0))
        else:
            # Compare the stages
            stage_score = _compare_stage_list(stage_type, exp_list, gen_list, result['details'])
...
def _compare_stage_list(stage_type: str, exp_list: List[KqlStage], gen_list: List[KqlStage], details: Dict) -> float:
...
def _compare_where_stages(exp_list: List[KqlStage], gen_list: List[KqlStage], details: Dict) -> float:
...
def _compare_summarize_stages(exp_list: List[KqlStage], gen_list: List[KqlStage], details: Dict) -> float:
...
def _compare_sort_stages(exp_list: List[KqlStage], gen_list: List[KqlStage], details: Dict) -> float:
...
def _compare_join_stages(exp_list: List[KqlStage], gen_list: List[KqlStage], details: Dict) -> float:
    ...
    # Combined score
    score = (0.3 * (1.0 if kind_match else 0.5) + 

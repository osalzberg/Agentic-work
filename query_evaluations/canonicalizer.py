...
_keyword_re = re.compile(r"\\b(" + "|".join(re.escape(k) for k in KEYWORDS) + r")\\b", re.IGNORECASE)
...
def canonicalize_kql(kql: str) -> str:
    ...
    s = re.sub(r"datetime\(([^)]+)\)", lambda m: f"datetime({m.group(1).strip()})", s, flags=re.IGNORECASE)
    return s

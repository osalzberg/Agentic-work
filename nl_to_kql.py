...
from azure_openai_utils import create_embeddings, emit_chat_event, run_chat
from prompt_builder import (build_prompt, get_system_prompt,  # type: ignore
                            stable_hash)
...
def load_domain_context(
    domain: str, nl_question: Optional[str] = None
) -> Dict[str, str]:
    ...
    for path in example_files:
        if os.path.exists(path):
            parsed_examples.extend(
                _parse_container_fewshots(path)
            )  # Reuse same parser (bold+fence format)
    ...
    capsule_excerpt = (
        _read_file(capsule_path, limit=600)
...
_CONTAINER_TABLE_REGEX = _re.compile(
    r"\b(containerlogv2|containerlog|kubepodinventory|kube[pP]od|insightsmetrics|containerinventory|kubeevents?)\b",
    _re.IGNORECASE,
)
_APP_TABLE_REGEX = _re.compile(
    r"\b(apprequests|appexceptions|apptraces|appdependencies|apppageviews|appcustomevents)\b",
    _re.IGNORECASE,
)
...
def detect_domain(nl_question: str) -> str:
    ...
    print(
        f"[domain-detect] q='{nl_question}' container_matches={sorted(matched_container)} app_matches={sorted(matched_app)}"
    )
    ...
    if matched_container and matched_app:
        if any(sig in matched_container for sig in ("<table-match>", "<pods-pending>")):
            print(
                "[domain-detect] chosen=containers (conflict; container strong signal)"
            )
            return "containers"
    ...
    print(
        "[domain-detect] no clear matches, defaulting to appinsights for generic monitoring queries"
    )
    return "appinsights"
...
def parse_container_function_signatures_with_docs(
    kql_text: str,
) -> List[Tuple[str, str]]:
...
def _parse_container_fewshots(path: str) -> List[Dict[str, str]]:
    ...
    while i < len(lines):
        ...
        if stripped.lower().startswith("q:"):
            ...
            if collecting:
                if not stripped and (
                    i + 1 < len(lines) and lines[i + 1].strip().lower().startswith("q:")
                ):
                    collecting = False
                else:
...
def _select_relevant_fewshots(
    nl_question: str, examples: List[Dict[str, str]]
) -> List[Dict[str, str]]:
    ...
    try:
        ...
        if any(
            tok in sample_text
            for tok in ["apprequests", "apptraces", "appexceptions", "appdependencies"]
        ):
            domain_guess = "appinsights"
        indexed = select_with_index(nl_question, examples, domain_guess, top_k=top_k)
        if indexed:
            print(
                f"[fewshot-select] used_index=True domain={domain_guess} returned={len(indexed)}"
            )
            return indexed
    ...
    try:
        _embed_inputs_preview = [nl_question] + [
            ex["question"] for _, ex in heuristic_records
        ]
        print(
            "[embed-inputs] total_inputs="
            + str(len(_embed_inputs_preview))
            + " preview="
            + json.dumps(_embed_inputs_preview[:8], ensure_ascii=False)[:800]
        )
    ...
    try:
        embeddings = _embed_texts(
            [nl_question] + [ex["question"] for _, ex in heuristic_records]
        )
    ...
    if not best_matching_examples:
        best_matching_examples = [
            ex for _, ex in hybrid_rows[: min(2, len(hybrid_rows))]
        ]
    print(
        f"[fewshot-select] embeddings_used=True (legacy) max_h={max_h} top_scores={[round(s,4) for s,_ in hybrid_rows[:3]]}"
    )
...
def load_container_shots(nl_question: Optional[str] = None) -> Dict[str, str]:
    ...
    if os.path.exists(csv_path):
        ...
        print(
            f"[container-public_shots] source=csv path={csv_path} count={len(public_shots_struct)}"
        )
    else:
        print(
            f"[container-public_shots] WARNING: CSV examples file missing at {csv_path}; proceeding with empty examples list"
        )
        public_shots_struct = []
    ...
    if selected_shots:
        ...
        fewshots_block = ""
        examples_error = (
            "No relevant container examples selected (index empty or low relevance)."
        )
        ...
        try:
            if public_shots_struct and nl_question:
                questions = [nl_question] + [
                    ex["question"] for ex in public_shots_struct
                ]
            ...
            print(
                f"[container-examples] candidate-score-diagnostics failed error={diag_exc}"
            )
        # Log diagnostic summary
        if top_scores:
            printable = [round(s, 4) for s, _ in top_scores]
            print(
                f"[container-examples] no-selection error='{examples_error}' top_candidate_scores={printable} total_available={len(public_shots_struct)}"
            )
        else:
            print(
                f"[container-examples] no-selection error='{examples_error}' top_candidate_scores=[] total_available={len(public_shots_struct)}"
            )
        return {
            "fewshots": fewshots_block,
            "capsule": "",  # suppressed
            ...
            "top_candidate_scores": json.dumps(
                [{"score": round(s, 6), "question": q} for s, q in top_scores]
            ),
        }
...
| order by $table asc"""
    if any(keyword in nl_lower for keyword in ["schema", "columns", "structure"]):
        ...
        mentioned_table = next(
            (t.title() for t in table_keywords if t in nl_lower), None
        )
        return (
            f"{mentioned_table} | getschema | project ColumnName, ColumnType"
            if mentioned_table
    ...
    for attempt in range(max_retries):
        ...
        if not result.startswith("// Error"):
            print(
                f"[retry] succeeded on attempt {attempt} with slim_prompt={use_slim_prompt}"
            )
            return result
        last_error = result
...
def _attempt_translation(nl_question, use_slim_prompt: bool = False):
    ...
    print(
        f"[prompt] schema_version={layered_meta.get('schema_version')} output_mode={layered_meta.get('output_mode')} system_hash={layered_meta.get('system_hash')} fn_index_hash={layered_meta.get('function_index_hash')}"
    )
    ...
    legacy_payload = {"system": system_prompt, "user": user_prompt}
    ...
    try:
        ...
        Exception
    ) as log_exc:  # defensive, translation should not fail due to logging

...
def _get_azure_credential():
    ...
    with _credential_creation_lock:
        ...
        print(
            "[Credential] Creating Azure credential (will trigger az login if needed)..."
        )
...
class SchemaManager:
    def __init__(self):
        ...
        self._manifest_cache_file = os.environ.get(
    ...
    def get_workspace_schema(self, workspace_id: str) -> Dict[str, Any]:
        ...
        with self._refresh_lock:
            ...
            print(
                f"[SchemaManager] Refresh start workspace={workspace_id} ttl_min={self._ttl_minutes}"
            )
            t_retrieve_start = time.time()
            tables, source = self._retrieve_tables(workspace_id)
            retrieve_dur = time.time() - t_retrieve_start
            print(
                f"[SchemaManager] Phase retrieval_done workspace={workspace_id} source={source} duration_s={retrieve_dur:.3f} table_count={len(tables)}"
            )
            # Backward-compatible log line for tests expecting legacy prefix
            print(
                f"[SchemaManager] Refresh retrieval_done workspace={workspace_id} source={source} duration_s={retrieve_dur:.3f}"
            )
            # Manifest loading deferred; log current state summary only.
            print(
                f"[SchemaManager] Manifest deferred loaded={self._manifest_loaded} tables_index={len(self._manifest_cache.get('resource_type_tables', {}))}"
            )
            ...
            table_resource_types: Dict[str, List[str]] = (
                self._manifest_cache.get("table_resource_types", {})
            ...
            table_primary_resource_type: Dict[str, str] = {
                t: (rts[0] if rts else "Unknown")
                for t, rts in table_resource_types.items()
            }
            # Enrichment
            ...
            print(
                f"[SchemaManager] Phase enrich_done workspace={workspace_id} duration_s={enrich_dur:.3f} enriched_count={len(enriched_tables)} total_refresh_s={total_dur:.3f}"
            )
            # Print out workspace tables (name + resource_type) after enrichment
            try:
                sample_list = [
                    f"{t.get('name')}({t.get('resource_type')})"
                    for t in enriched_tables
                ]
                joined = ", ".join(sample_list)
                print(
                    f"[WorkspaceTables] --------- workspace={workspace_id} total={len(enriched_tables)} tables={joined}"
                )
    ...
    def load_manifest(self, force: bool = False) -> None:
        with self._manifest_lock:
            ...
            if not force and os.path.exists(self._manifest_cache_file):
                try:
                    ...
                    if isinstance(data, dict) and data.get("resource_type_tables"):
                        ...
                        print(
                            f"[Manifest] Loaded persisted manifest cache file={self._manifest_cache_file} resource_types={len(data.get('resource_type_tables', {}))}"
                        )
                        return
                ...
                for root, dirs, files in os.walk(base_dir):  # type: ignore[attr-defined]
                            ...
                            try:
                                ...
                                def _walk(obj):
                                    if isinstance(obj, dict):
                                        for k, v in obj.items():
                                            if k.lower() == "tables" and isinstance(
                                                v, list
                                            ):
                                                for t in v:
                                                    if isinstance(t, dict):
                                                        nm = t.get("name") or t.get(
                                                            "tableName"
                                                        )
                                                    elif isinstance(t, str):
                                                        nm = t
                                ...
                                if isinstance(rtype, str) and "/" in rtype:
                                    ...
                                    for tname in sorted(set(tbls)):
                                        ...
                                        table_resource_types.setdefault(
                                            tname, []
                                        ).append(rtype)
                                manifests_scanned += 1
                    ...
                    for f in files:
                        if f.endswith("_kql_examples.md"):
                            ...
                            mapping.setdefault(rtype, []).append(
                                f.replace("_kql_examples.md", "")
                            )
            ...
            try:
                ...
                print(
                    f"[Manifest] Persisted manifest cache file={self._manifest_cache_file} size_rt={len(mapping)} tables={len(table_resource_types)}"
                )
            except Exception as e:  # pragma: no cover
    ...
    def _rest_list_tables(self) -> list[Dict[str, Any]]:
        ...
        try:
            ...
            token = _azure_credential.get_token(
            ...
            if resp.status_code != 200:
                print(
                    f"[SchemaManager] REST list tables failed: {resp.status_code} {resp.text[:200]}"
                )
                return []
            data = resp.json()
            ...
            print(
                f"[SchemaManager] REST list done tables={len(tables)} duration_s={time.time()-t0:.3f}"
            )
            return tables
        except Exception as e:  # pragma: no cover
    ...
    def _union_enumerate_tables(self, workspace_id: str) -> list[Dict[str, Any]]:
        ...
        try:
            ...
            t0 = time.time()
            print(
                f"[SchemaManager] Union enumeration start workspace={workspace_id} timespan_days=7"
            )
            exec_result = execute_kql_query(kql=query, workspace_id=workspace_id, client=client, timespan=timedelta(days=7))
            tables: list[Dict[str, Any]] = []
            for tbl in exec_result.get("tables", []):
                name = tbl.get("name") if isinstance(tbl, dict) else None
                if name:
                    tables.append({"name": str(name)})
            print(
                f"[SchemaManager] Union enumeration done tables={len(tables)} duration_s={time.time()-t0:.3f}"
            )
            return tables
        except Exception as e:  # pragma: no cover
